...
class Polynomial :
    ...
    def __mul__(self, rhsPoly):
        ...
        node = self._polyHead
        newPoly = rhsPoly._help_mul(node)
        node = node.next
        while node is not None:
            curr = rhsPoly._help_mul(node)
            newPoly += curr
            node = node.next
        return newPoly

    def _help_mul(self, node):
        newPoly = Polynomial()
        curr = self._polyHead
        while curr is not None:
            degree = curr.degree + node.degree
            coefficient = curr.coefficient * node.coefficient
    ...
    def __str__(self):
        ...
        prob = self._polyHead
        while prob:
            result += '{} {}\n'.format(str(prob.degree), str(prob.coefficient))
            prob = prob.next    
        return result
